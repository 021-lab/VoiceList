...
Статус: rev 1. Скоуп v1: отображение графа + фронтир; focus/pause и смена статусов.
Канон: TAOSMD_API_CONTRACT.md и TASK_LIFECYCLE.md живут в форке `021-lab/taosmd` (docs/); ссылки на них — по PINNED_COMMIT SHA. Этот документ описывает только интеграцию фронта.
...
/deploy/         — README деплоя: Pages, wrangler, пример конфига cloudflared
```
...
1. **Провенанс**: каждая мутация фронта — `created_by=user`. Как HTTP-слой принимает `created_by` (параметр тела или заголовок) — берётся из результатов шага 0 контракт-задания; если сервер значение не принимает, его добавляет Worker-proxy. Оставлять серверный дефолт запрещено.
2. **Статусы**: enum `{open, in_progress, blocked, closed, superseded, focus, pause}` (амендмент rev 2 форка, ветка `feat/user-statuses`). `focus`/`pause` ставит только пользователь; фронт не ставит их на задачи в `in_progress`.
...
D. **Деплой** (`/deploy`): Pages-проект на recorder.smileme.ai, `wrangler deploy` Worker'а с route, пример конфига cloudflared (ingress → 127.0.0.1:7900), порядок включения Cloudflare Access.
...
- Ветка `feat/user-statuses` форка (статусы focus/pause + `tasks reset`) задеплоена на Mac Mini.
